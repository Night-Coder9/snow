...
const PipelineView = ({
  ...
  const onDragEnd = (dropResult: DropResult) => {
    ...
    switch (type) {
      case 'lane': {
        const newLanes = [...allLanes].splice(source.index, 1)
          .splice(destination.index, 0, allLanes[source.index])
          .map((lane: any, idx: any) => {
      ...
      case 'ticket': {
        ...
        if (source.droppableId === destination.droppableId) {
          const newOrderedTickets = [...originLane.Tickets]
            ...
            .splice(destination.index, 0, originLane.Tickets[source.index])
