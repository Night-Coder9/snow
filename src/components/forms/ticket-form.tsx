...
const TicketForm = ({ getNewTicket, laneId, subaccountId }: Props) => {
  ...
  const [assignedTo, setAssignedTo] = useState(
    defaultData.tickets?.Assigned?.id || ''
  )
  const form = useForm<z.infer<typeof TicketFormSchema>>({
    mode: 'onChange',
    resolver: zodResolver(TicketFormSchema),
    defaultValues: {
      name: defaultData.tickets?.name || '',
      description: defaultData.tickets?.description || '',
      value: String(defaultData.tickets?.value || 0),
    },
  })
  ...
  useEffect(() => {
    if (defaultData.tickets) {
      form.reset({
        name: defaultData.tickets.name || '',
        description: defaultData.tickets?.description || '',
        value: String(defaultData.tickets?.value || 0),
      })
      if (defaultData.tickets.customerId)
        setContact(defaultData.tickets.customerId)

      const fetchData = async () => {
        const response = await searchContacts(
          //@ts-ignore
          defaultData.tickets?.Customer?.name
        )
        ...
        <Form {...form}>
          ...
          >
            ...
            <TagCreator
              ...
              defaultTags={defaultData.tickets?.Tags || []}
            />
